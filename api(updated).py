...
try:
    ...
    print("[WARN] captcha_solver.py not found – captcha solving disabled")
...
# ─── Upgraded Request Handler with Captcha Support ──────────────────────────
...
async def make_graphql_request_with_captcha_handling(
    ...
    """
    Send a GraphQL request and automatically handle captcha challenges.
    If a captcha is required, it will be solved and the request retried.
    """
        ...
        try:
            ...
            if solve_captcha and CAPTCHA_SOLVER_AVAILABLE and is_captcha_required(response_text):
                ...
                result = await solve_shopify_captcha(
                    ...
                    captcha_type="auto",  # auto‑detect provider
                    sitekey=None
                )
                
                if result.success:
                    print(f"[shopify] Captcha solved: {result.provider} - {result.method}")
                    # Inject the token into the captcha field
                    ...
                    print(f"[shopify] Captcha solving failed: {result.error}")
                    return None, f"CAPTCHA_SOLVE_FAILED: {result.error}", False
            
            # No captcha, or solved successfully
        ...
        for _fetch_retry in range(2):
            ...
            async with session.get(f"{domain}/products.json?limit=250", proxy=proxy, timeout=10) as resp:
                ...
                if trimmed[0] not in ('{', '['):
                    ...
                    print(f"[shopify] fetch_products non-JSON response, preview: {trimmed[:200]}")
                ...
                try:
                    ...
                    print(f"[shopify] fetch_products JSON decode failed, preview: {preview}")
                    if _is_proxy_error(raw_text):
                        return False, "<b>Proxy Error: bad proxy response</b>"
...
async def process_card(cc, mes, ano, cvv, site_url, variant_id=None, proxy_str=None, session=None):
    ...
    try:
        ...
        try:
            ...
            print(f"[shopify] process_card session error: {type(e).__name__}: {repr(e)}")
            return False, "Proxy Error", gateway, total_price, currency
        except asyncio.TimeoutError:
            return False, "Proxy Error", gateway, total_price, currency
        except Exception as e:
            print(f"[shopify] process_card inner exception: {type(e).__name__}: {repr(e)}")
